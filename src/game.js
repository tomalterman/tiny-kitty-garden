...
function buildRooms() {
    return {
        garden: {
            ...
            yarnBall: { x: 220, y: 170, collected: false, rolling: false },
            starsSpawned: false,
            stars: []
        },
        kitchen: {
            ...
            yarnBall: { x: 60, y: 170, collected: false },
            starsSpawned: false,
            stars: [],
            treatsOffered: 0
        },
        bedroom: {
            ...
            collectibles: [],
            yarnBall: { x: 90, y: 170, collected: false },
            toyBoxFill: 0,
            starsSpawned: false,
            stars: []
        },
...
function makeGardenCollectibles() {
    // 5 flowers with unique colors at fixed positions
    const positions = [
        { x: 40,  y: 180 },
        { x: 160, y: 175 },
        { x: 200, y: 190 },
...
function makeBeachCollectibles() {
    const positions = [
        { x: 30,  y: 175 }, { x: 70,  y: 190 }, { x: 130, y: 180 },
        { x: 200, y: 190 }, { x: 260, y: 175 }, { x: 340, y: 185 }
    ];
...
function drawGarden(ctx) {
    // Grass strip
    ctx.fillStyle = PAL.grass;
    ctx.fillRect(0, 140, GAME.width, GAME.height - 140);
    ctx.fillStyle = PAL.grassDk;
    ctx.fillRect(0, 140, GAME.width, 2);
    // Background trees
    drawTree(ctx, 20, 150);
    drawTree(ctx, 360, 150);
    drawTree(ctx, 300, 145);
    // Bushes
    drawBush(ctx, 110, 145);
    drawBush(ctx, 250, 148);
    // Pond shimmer
    const room = world.rooms.garden;
    const pond = room.hotspots.find(h => h.kind === 'pond');
    drawPond(ctx, pond);
    // Background micro-flowers (non-collectible)
    for (let i = 0; i < 10; i++) {
...
function drawKitchen(ctx) {
    ...
    ctx.fillRect(0, PLAY_TOP, GAME.width, 130 - PLAY_TOP);
    ctx.fillStyle = PAL.peachDk;
    ctx.fillRect(0, 128, GAME.width, 2);
    ctx.fillStyle = PAL.cream;
    ctx.fillRect(0, 130, GAME.width, GAME.height - 130);
    // Floor tiles
    ctx.fillStyle = PAL.peach;
    for (let x = 0; x < GAME.width; x += 20) {
        for (let y = 135; y < GAME.height; y += 20) {
    ...
    // Rug
    ctx.fillStyle = PAL.red;
    ctx.fillRect(100, 150, 110, 40);
    ctx.fillStyle = PAL.yellow;
    ctx.fillRect(100, 150, 110, 2);
    ctx.fillRect(100, 188, 110, 2);
    for (let i = 0; i < 11; i++) {
        ctx.fillRect(100 + i * 10, 155, 1, 30);
    }
    // Mouse hole
    ctx.fillStyle = PAL.outline;
    ctx.beginPath();
    ctx.ellipse(320, 185, 8, 6, 0, Math.PI, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = PAL.woodDk;
    ctx.fillRect(312, 190, 16, 3);
    // Wall decoration
    drawKittenPortrait(ctx, 50, 50);
}

function drawBedroom(ctx) {
    // Wall
    ctx.fillStyle = PAL.lav;
    ctx.fillRect(0, PLAY_TOP, GAME.width, 135 - PLAY_TOP);
    // Wall pattern - tiny stars
    ctx.fillStyle = PAL.lavDk;
    for (let i = 0; i < 20; i++) {
        const x = (i * 29 + 5) % GAME.width;
        const y = PLAY_TOP + 10 + (i % 5) * 18;
    ...
    // Floor
    ctx.fillStyle = PAL.sky;
    ctx.fillRect(0, 135, GAME.width, GAME.height - 135);
    ctx.fillStyle = PAL.skyDk;
    ctx.fillRect(0, 135, GAME.width, 2);
    for (let x = 0; x < GAME.width; x += 24) {
        ctx.fillRect(x, 140, 1, GAME.height - 140);
    }
    // Bed
    drawBed(ctx, world.rooms.bedroom.hotspots.find(h => h.kind === 'bed'));
    // Toy box
    drawToyBox(ctx, 30, 160, world.rooms.bedroom.toyBoxFill);
    // Window
    ctx.fillStyle = PAL.sky;
    ctx.fillRect(140, 40, 40, 30);
    ctx.fillStyle = PAL.outline;
    ctx.fillRect(140, 40, 40, 1);
    ctx.fillRect(140, 70, 40, 1);
    ctx.fillRect(140, 40, 1, 30);
    ctx.fillRect(180, 40, 1, 30);
    ctx.fillRect(158, 40, 1, 30);
    ctx.fillRect(140, 55, 40, 1);
}
...
function drawTree(ctx, x, y) {
    ...
    ctx.fillStyle = PAL.grassDk;
    ctx.beginPath();
    ctx.arc(x - 4, y - 22, 5, 0, Math.PI * 2);
    ctx.fill();
}
...
function drawToyBox(ctx, x, y, fill) {
    ...
    ctx.fillStyle = PAL.yellow;
    ctx.fillRect(x + 12, y + 8, 10, 6);
    ctx.fillStyle = PAL.outline;
    ctx.font = 'bold 4px monospace';
    // Filled yarn balls peek out
    for (let i = 0; i < fill; i++) {
        drawYarnBall(ctx, x + 4 + i * 7, y - 2, i, true);
    }
}
...
function triggerHotspot(hs, room) {
    ...
    if (hs.kind === 'pond') {
        // Frog hops regardless of kitten position
        hs.state = 30;
        Sound.play('ribbit');
        ...
        // Splash happens over wherever the kitten ends up on the wet sand
...
function updateHotspots(dt) {
    ...
    for (const hs of room.hotspots) {
        // Kitchen mouse first-entry wave
        if (hs.kind === 'mouse' && !hs.waved) {
            hs.state = 90;
            hs.waved = true;
            // After wave, drop a treat. Re-check the cap inside the callback
            // so rapid re-entries can't over-fill past 4.
            setTimeout(() => {
